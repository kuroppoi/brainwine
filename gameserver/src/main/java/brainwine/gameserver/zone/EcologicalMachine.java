...
public enum EcologicalMachine {
    ...
    public static EcologicalMachine fromPart(Item part) {
        return Stream.of(values()).filter(x -> x.isMachinePart(part)).findFirst().orElse(fromBase(part));
    }
    ...
    public boolean isMachinePart(Item item) {
        return item.hasId(base) || parts.contains(item.getId());
    }
    ...
    public List<Item> getParts() {
        return parts.stream().map(ItemRegistry::getItem).collect(Collectors.toList());
    }
}
